...
fs::path path_check_exists(const fs::path& dir, const std::string& fname) {
	auto path = dir / fname;
	if (fs::exists(path)) {
		std::cout << path.string() << " zaten var, degistirmek ister misiniz? [y/N] ";
		if (!get_yes_no()) {
			std::cout << "Lutfen alternatif bir dosya adi girin: " << std::endl;
			std::string newFname;
...
const fs::path pathsIniPath = program_path() / "Tools\\PathsYRMU.ini";
...
int main(int argc, const char** argv) {
	...
	if (!(fs::exists(ptmp1) && fs::exists(ptmp2) && fs::exists(ptmp3))) {
		std::cout << "Enter full path to AeFNeT: " << std::endl;
		std::string newPath;
		...
		while (!(fs::exists(ptmp1) && fs::exists(ptmp2) && fs::exists(ptmp3))) {
			std::cout << "AeFNeT dizinleri bulunamadi, AeFNeT'e tam yol girin: " << std::endl;
			std::getline(std::cin >> std::ws, newPath);
			ptmp1 = fs::path(newPath);
			ptmp2 = ptmp1 / mapsPathRelative;
			ptmp3 = ptmp1 / "INI\\MPMaps.ini";
	...
	std::cout << "Yeni haritalarin ve onizlemelerin bir listesini olusturmak ister misiniz? [y/N] ";
	if (get_yes_no()) {
		...
		if (!(fs::exists(configPath) && configPath.filename() == "versionconfig.ini")) {
			std::cout << "versionconfig.ini Ýcin tam yolu girin:" << std::endl;
			std::string newPath;
			std::getline(std::cin >> std::ws, newPath);
			configPath = fs::path(newPath);
			while (!(fs::exists(configPath) && configPath.filename() == "versionconfig.ini")) {
				std::cout << "Gecersiz yol, lutfen tekrar deneyin:" << std::endl;
				std::getline(std::cin >> std::ws, newPath);
				configPath = fs::path(newPath);
			}
		...
		newMaps.close();
		std::cout << "Yeni haritalarin ve onizlemelerin listesi olusturuldu... " << outPath.string() << std::endl;
	}
	...
	if (!missing.empty()) { // if any maps were missing names, write them all to a file
		...
		std::cout << "Devam etmek ister misin? Adi eksik olan haritalar islenmeyecek [y/N] ";
		if (!get_yes_no())
			return 0;
	}
	...
	while (!fs::exists(mpmapsBasePath) || mpmapsBasePath.filename() != "MPMapsBase.ini") {
		std::cout << "MPMapsBase.ini bulunamadi, lutfen tam yolu girin:" << std::endl;
		std::string inputTemp;
		std::getline(std::cin >> std::ws, inputTemp);
		mpmapsBasePath = fs::path(inputTemp);
	}
	if (fs::exists(mpmapsPath)) {
		std::cout << mpmapsPath.string() << " zaten var, silmek ister misiniz? [y/N] ";
		if (get_yes_no())
	...
	std::cout << "MPMaps.ini olusturuldu...";
	if (!notes.empty())
		std::cout << ", eksik verilerle ilgili notlar mpmaps.ini dosyasinin sonuna yazilmistir...";
	std::cout << std::endl;
